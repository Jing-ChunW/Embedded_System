...
void uLCD_4DGL ::freeBUFFER(void) // Clear serial buffer before writing command
{
    ...
    {
        char recv[1];
        _cmd.read(recv, sizeof(recv)); // clear buffer garbage
    }
}
...
int uLCD_4DGL ::writeCOMMAND(char *command, int number) // send several BYTES making a command and return an answer
{
    ...
    while (!_cmd.readable())
        ThisThread::sleep_for(TEMPO * 1ms); // wait for screen answer
    if (_cmd.readable())
    {
        char recv[1];
        resp = _cmd.read(recv, sizeof(recv));
...
void uLCD_4DGL ::reset() // Reset Screen
{
    ...
    ThisThread::sleep_for(5ms);    // wait a few milliseconds for command reception
    _rst = 1;         // put RESET back to high
    ThisThread::sleep_for(3000ms); // wait 3s for screen to restart
...
void uLCD_4DGL ::baudrate(int speed) // set screen baud rate
{
    ...
    {
        ThisThread::sleep_for(TEMPO * 1ms); // wait for screen answer - comes 100ms after change
...
int uLCD_4DGL ::readVERSION(char *command, int number) // read screen info and populate data
{
    ...
    for (i = 0; i < number; i++)
        writeBYTE(command[i]); // send all chars to serial port
    ...
    {
        char recv[1];
        temp = _cmd.read(recv, sizeof(recv));
